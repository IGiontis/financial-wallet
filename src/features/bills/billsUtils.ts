...
// ─── What a bill usually costs, and whether the last one broke it ────────────
// The one question the app stores the answer to and has never shown.
//
// Every payment of a variable bill records what actually left — electricity at
// €48 in May and €88 in January — and that history sat in the database being
// used for nothing but an average. What it can answer is "has this got dearer",
// which is not something any other screen on the page can say.
//
// Deliberately not a trend line or a percentage against last year. Utilities are
// seasonal: January is dearer than May every year and calling that a rise is
// noise. The claim made here is narrower and survives the seasons — this charge
// is outside every charge that came before it, or it is not.

/** One charge, as it is drawn. */
export interface BillCharge {
  amount: number;
  date: Date;
}

export interface BillSpread {
  bill: BillWithStatus;
  /** Oldest first, so the bars read left to right. */
  charges: BillCharge[];
  latest: BillCharge;
  /** Every charge before the latest. Absent when there are too few to speak of. */
  usual?: { min: number; max: number };
  /** Where the latest sits against `usual`. */
  standing?: "above" | "usual" | "below";
  /** How far outside it fell, when it did. Always positive. */
  gap?: number;
}

/** How many charges are drawn. A year of a monthly bill. */
const SPREAD_WINDOW = 12;
/** Below this there is no shape to look at. */
const MIN_CHARGES = 3;
/** A verdict needs something to compare against — this many charges before the latest. */
const MIN_HISTORY = 3;

/**
 * Every bill with enough history to have a usual cost, dearest surprise first.
 *
 * A bill whose charges are all the same amount is left out: it has no spread,
 * and a row saying "€25 every time, as always" is a row that says nothing. What
 * belongs here is the bills that move.
 */
export function billSpreads(bills: BillWithStatus[], window = SPREAD_WINDOW): BillSpread[] {
  const spreads: BillSpread[] = [];

  for (const bill of bills) {
    // `payments` arrives newest first; the bars want the other order.
    const charges = bill.payments
      .slice(0, window)
      .map((payment) => ({ amount: payment.amount, date: firestoreToDate(payment.paidDate) }))
      .reverse();

    if (charges.length < MIN_CHARGES) continue;
    if (charges.every((charge) => charge.amount === charges[0].amount)) continue;

    const latest = charges[charges.length - 1];
    const before = charges.slice(0, -1).map((charge) => charge.amount);

    if (before.length < MIN_HISTORY) {
      spreads.push({ bill, charges, latest });
      continue;
    }

    const usual = { min: Math.min(...before), max: Math.max(...before) };
    const standing = latest.amount > usual.max ? "above" : latest.amount < usual.min ? "below" : "usual";
    const gap = standing === "above" ? round2(latest.amount - usual.max) : standing === "below" ? round2(usual.min - latest.amount) : undefined;

    spreads.push({ bill, charges, latest, usual, standing, gap });
  }

  // The ones that broke their range first, by how badly; then the rest, dearest
  // first. A page called "what got dearer" should open on what got dearer.
  const rank = (spread: BillSpread) => (spread.standing === "above" ? 0 : spread.standing === "below" ? 2 : 1);
  return spreads.sort((a, b) => rank(a) - rank(b) || (b.gap ?? 0) - (a.gap ?? 0) || b.latest.amount - a.latest.amount);
}
