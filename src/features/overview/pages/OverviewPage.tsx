...
const getDateRange = (period: TimePeriod): { start: Date; end: Date } => {
  const now = new Date();
  switch (period) {
    case "current_month":  return { start: startOfMonth(now), end: now };
    case "last_3_months":  return { start: startOfMonth(subMonths(now, 2)), end: now };
    case "last_6_months":  return { start: startOfMonth(subMonths(now, 5)), end: now };
...
const groupByWeek = (transactions: Transaction[], range: { start: Date; end: Date }): ChartDataPoint[] => {
  ...
  return weeks.map((w) => {
    const weekTx = transactions.filter((tx) => { const d = firestoreToDate(tx.date); return d >= w.start && d <= w.end; });
    return {
      label:    w.label,
...
const groupByMonth = (transactions: Transaction[]): ChartDataPoint[] => {
  ...
  transactions.forEach((tx) => {
    if (tx.type === "investment") return; // exclude investment from chart bars
...
const calculateMetrics = (transactions: Transaction[]): DashboardMetrics => {
  // Exclude investment transactions from income/expense calculations
  ...
  return { totalIncome, totalExpenses, totalInvestments, netIncome, savingsRate };
};
    ...
    return (
      <div style={{ background: "#1e1e2e", border: "none", borderRadius: 10, padding: "12px 16px", minWidth: 180, boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", display: "flex", alignItems: "center", gap: 7 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: "#F87171", display: "inline-block" }} />Expenses
          </span>
...
function RoundedBar(props: any) {
  ...
  return (
    <path d={`M${x + radius},${y} h${width - 2 * radius} a${radius},${radius} 0 0 1 ${radius},${radius} v${height - radius} h${-width} v${-(height - radius)} a${radius},${radius} 0 0 1 ${radius},${-radius}z`} fill={fill} />
  );
}

function MetricCard({ label, value, color, isPercentage = false, formatFn }: {
  label: string; value: number; color: string; isPercentage?: boolean; formatFn?: (n: number) => string;
}) {
...
export const OverviewPage: React.FC = () => {
  ...
  if (txLoading) {
    return <div className="d-flex justify-content-center align-items-center" style={{ minHeight: 300 }}><Spinner color="primary" /></div>;
  }
  if (txError) {
    return <Container fluid className="py-4"><p style={{ color: "var(--bs-danger)", fontSize: 14 }}>Failed to load overview. Please refresh.</p></Container>;
  }
    ...
    <Container fluid className="py-2">
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, background: "var(--color-background-secondary)", borderRadius: 10, padding: 4 }}>
          {PERIODS.map((p) => (
            <button key={p.value} onClick={() => handlePeriodChange(p.value)} disabled={isPending}
              style={{ background: selectedPeriod === p.value ? "var(--color-background-primary)" : "transparent", border: "none", borderRadius: 7, padding: "6px 12px", fontSize: 12, fontWeight: selectedPeriod === p.value ? 600 : 400, color: selectedPeriod === p.value ? "var(--color-text-primary)" : "var(--color-text-secondary)", cursor: "pointer", boxShadow: selectedPeriod === p.value ? "0 1px 4px rgba(0,0,0,0.08)" : "none", transition: "all 0.15s ease", whiteSpace: "nowrap" }}>
              {p.label}
            </button>
          ))}
        </div>
      ...
      <Row className="g-3 mb-4" style={{ opacity: isPending ? 0.5 : 1, transition: "opacity 0.2s" }}>
        ...
        <Col xs={6} lg>
          <MetricCard label="Invested"       value={metrics.totalInvestments} color="#818CF8" formatFn={formatCurrency} />
        </Col>
        <Col xs={6} lg>
              ...
              <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "var(--color-text-secondary)", margin: 0 }}>
                    {selectedPeriod === "current_month" ? "Weekly breakdown" : "Monthly breakdown"}
                  </p>
                </div>
                <div className="d-flex gap-3">
                  {[{ color: "#22C55E", label: "Income" }, { color: "#F87171", label: "Expenses" }].map((l) => (
                ...
                <ResponsiveContainer width="100%" height={280} debounce={200}>
                  <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }} barGap={3} barCategoryGap="30%">
                    ...
                    <Bar dataKey="income" name="Income" maxBarSize={40} shape={<RoundedBar />}>
                      {chartData.map((_, i) => <Cell key={i} fill="#22C55E" fillOpacity={0.85} />)}
                    </Bar>
