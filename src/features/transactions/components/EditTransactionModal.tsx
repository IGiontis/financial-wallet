...
export default function EditTransactionModal({ transaction, isOpen, onClose, categories, onSubmit }: EditTransactionModalProps) {
  ...
  const formik = useFormik<EditTransactionFormValues>({
    ...
    initialValues: {
      amount: displayAmount, type: editableType, categoryId: transaction.categoryId,
      date: toDateInputValue(transaction.date), description: transaction.description,
  ...
  return (
    <Modal isOpen={isOpen} toggle={handleClose} size="md">
      ...
      {step === "review" ? (
        <ReviewScreen
          values={formik.values} categories={categories}
          onBack={() => setStep("form")} onConfirm={() => formik.submitForm()}
          isSubmitting={formik.isSubmitting} formatAmount={formatAmount}
        />
      ) : (
        <>
          <ModalBody>
                ...
                <Col xs={6}>
                  <FormGroup className="mb-0">
                    <Label style={{ fontSize: 13, fontWeight: 500 }}>
                      ...
                      {formik.values.showFuelDetails && <span style={{ fontSize: 11, color: "#3B82F6", marginLeft: 6 }}>auto-calculated</span>}
                    </Label>
                    <Input
